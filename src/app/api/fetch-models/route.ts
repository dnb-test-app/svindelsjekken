          ...
          messages: [
            {
              role: "system",
              content:
                "You are a fraud detection expert. Always respond in valid JSON format only.",
            },
            {
              role: "user",
    ...
    if (responseNoJson.ok) {
      ...
      if (content) {
        ...
        try {
          ...
          const responseWithJson = await fetch(
            "https://openrouter.ai/api/v1/chat/completions",
            {
              method: "POST",
              headers: {
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const results: ModelTestResult[] = textModels.map(
      (model: OpenRouterModel) => {
    ...
    return NextResponse.json({
      ...
      statistics: {
        ...
        averageScore: Math.round(
          modelsWithScore.reduce((sum, m) => sum + m.performanceScore, 0) /
            modelsWithScore.length,
        ),
        withJsonSupport: modelsWithScore.filter((m) => m.supportsJson).length,
        withStructuredOutput: modelsWithScore.filter(
          (m) => m.supportsStructuredOutput,
        ).length,
        withNativeJSONSchema: modelsWithScore.filter(
          (m) => m.supportsNativeJSONSchema,
        ).length,
